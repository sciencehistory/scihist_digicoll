...
class GoogleArtsAndCultureDownloadCreator
  MAX_RETRY_COUNT = 10
  ...
  def find_or_create_record(retry_count: 0)
    if retry_count > MAX_RETRY_COUNT
      ...
      raise StandardError.new("Tried to find/create a GoogleArtsAndCultureDownloadCreator record too many times for work #{id}")
    end
    ...
    if record.nil?
      ...
      #record = GoogleArtsAndCultureDownloadCreator.create!(work_id: work.id, deriv_type: derivative_type, status: :in_progress, inputs_checksum: calculated_checksum)
      #GoogleArtsAndCultureDownloadCreatorJob.perform_later(work, derivative_type)
      record = GoogleArtsAndCultureDownload.new()
    end
