...
class OrphanS3VideoDerivatives
  ...
  def all_hls_deriv_folders
    ...
    s3_iterator.each_s3_path do |all_derivs_for_asset|
      ...
      if /\/$/.match? all_derivs_for_asset
        #The path ends in a slash, so we consider it a folder and iterate again over its contents.
        S3PathIterator.new(
        ...
        # This is a single stray file directly under the /hls directory.
        # Let's yield it so it can be flagged as an orphan.
        yield all_derivs_for_asset
      end
    end
  end
