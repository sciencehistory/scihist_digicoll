module OralHistory
  ...
  # Legacy component for displaying old circa Jan 2026 AI conversations,
  # with answer_json with `footnotes` and `narrative` keys.
  #
  # Can be deleted when we don't need those anymore.
  class AiConversationDisplayJan2026 < ApplicationComponent
    ...
    def answer_narrative
      @answer_narrative ||= begin
        if ai_conversation.llm_says_answer_unavailable?
          ...
          format_footnote_reference_html(@ai_conversation.answer_json&.dig("narrative"))
        end
      end
    end
    ...
    def build_footnote_data
      chunk_ids = @ai_conversation.answer_json&.dig("footnotes")&.collect {|h| h["chunk_id"]}
      return [] unless chunk_ids

      @ai_conversation.answer_json&.dig("footnotes").collect do |response_hash|
