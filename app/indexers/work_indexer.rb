...
class WorkIndexer < Kithe::Indexer
  ...
  field_map = {
    :model_pk_ssi =>      ["model_pk_ssi"],
    :title =>             ["text1_tesim"],
    :additional_title =>  ["text1_tesim"],
    :creator =>           ["text2_tesim","creator_facet"],
    :genre =>             ["text2_tesim", "genre_facet"],
    :subject =>           ["text3_tesim", "subject_facet"],
    :interviewer =>       ["interviewer_facet"],

    # index description to its own field for highlighting purposes. Fields with
    # HTML in them need to have it stripped before indexing.
    :description =>       ["description_text4_tesim"],

    :provenance =>        ["text4_tesim"],
    :language =>          ["text_no_boost_tesim", "language_facet"],
    :external_id =>       ["text_no_boost_tesim"],
    :related_url =>       ["text_no_boost_tesim"],
    :rights =>            ["text_no_boost_tesim", "rights_facet"],
    :rights_label =>      ["text_no_boost_tesim"],
    :rights_holder =>     ["text_no_boost_tesim"],
    :series_arrangement =>["text_no_boost_tesim"],
    :inscription =>       ["text_no_boost_tesim"],
    :additional_credit => ["text_no_boost_tesim"],
    :digitization_funder =>["text_no_boost_tesim"],
    :extent =>            ["text_no_boost_tesim"],
    :physical_container =>["text_no_boost_tesim"],

    # We put these two in a separate field,
    # cause we only allow logged in users
    # to search them
    :admin_note =>        ["admin_only_text_tesim"],
    :exhibition =>        ["admin_only_text_tesim", "exhibition_facet"],

    :expanded_years =>    ["year_facet_isim"],
    :earliest_date =>     ["earliest_date"],
    :latest_date =>       ["latest_date"],
    
    :collection_id =>     ["collection_id_ssim"],
    
    :created_at =>        ["date_created_dtsi"],
    :updated_at =>        ["date_modified_dtsi"],
    :published_at =>      ["date_published_dtsi"],

    :published_boolean => ["published_bsi"],

    :oh_feature_facet =>        ["oh_feature_facet"],
    :oh_availability_facet =>   ["oh_availability_facet"],
    :oh_institution_facet =>    ["oh_institution_facet"],
    :oh_birth_country_facet =>  ["oh_birth_country_facet"],
    :bredig_feature_facet =>    ["bredig_feature_facet"],

    :place =>             ["text_no_boost_tesim", "place_facet"],
    :department =>        ["text_no_boost_tesim", "department_facet"], 
    :medium =>            ["text_no_boost_tesim", "medium_facet"],
    :format =>            ["text_no_boost_tesim", "format_facet"],

    :fulltext_en =>       ["searchable_fulltext_en"],
    :fulltext_de =>       ["searchable_fulltext_de"],
  ...
  configure do
    ...
    to_field field_map[:title],             obj_extract("title")
    to_field field_map[:additional_title],  obj_extract("additional_title")
    to_field field_map[:creator],           obj_extract("creator", "value")
    to_field field_map[:genre],             obj_extract("genre")
    to_field field_map[:subject],           obj_extract("subject")

    # Interviewer out of creator facet for use specifically for Oral History collection
    to_field field_map[:interviewer] do |record, acc|
      acc.concat record.creator.find_all { |creator| creator.category == "interviewer"}.collect(&:value)
    end

    to_field field_map[:description],
      obj_extract("description"), transform(->(val) { ActionView::Base.full_sanitizer.sanitize(val) })
    to_field field_map[:provenance],
      obj_extract("provenance"), transform(->(val) { ActionView::Base.full_sanitizer.sanitize(val) })

    to_field field_map[:language],    obj_extract("language")
    to_field field_map[:external_id], obj_extract("external_id", "value")
    to_field field_map[:related_url], obj_extract("related_url")
    to_field field_map[:place],       obj_extract("place", "value")
    to_field field_map[:department],  obj_extract("department")
    to_field field_map[:medium],      obj_extract("medium")
    to_field field_map[:format],      obj_extract("format"), transform(->(v) { v.titleize })
    to_field field_map[:rights],      obj_extract("rights") # URL id

    to_field field_map[:rights_label],
      obj_extract("rights"), transform(->(v) { RightsTerm.label_for(v) }) # human label

    to_field field_map[:rights_holder],       obj_extract("rights_holder")
    to_field field_map[:series_arrangement],  obj_extract("series_arrangement")

    to_field field_map[:inscription],
      obj_extract("inscription"), transform(->(v) { "#{v.location}: #{v.text}" })

    to_field field_map[:additional_credit],
      obj_extract("additional_credit"), transform(->(v) { "#{v.role}: #{v.name}" })

    to_field field_map[:digitization_funder], obj_extract("digitization_funder")
    to_field field_map[:extent], obj_extract("extent")

    to_field field_map[:physical_container],
      obj_extract("physical_container"), transform( ->(v) { v.display_as })


    to_field field_map[:admin_note], obj_extract("admin_note")
    to_field field_map[:exhibition], obj_extract("exhibition")

    # for date/year range facet
    to_field field_map[:expanded_years] do |record, acc|
      acc.concat(DateIndexHelper.new(record).expanded_years)
    end

    # For sorting by oldest first
    to_field field_map[:earliest_date]  do |record, acc|
    ...
    to_field field_map[:latest_date] do |record, acc|
      # for Solr, we need in "xml schema" format, with 00:00:00 time, and UTC timezone
      # Rails date extensions are confusing, but this works to get it.
    ...
    to_field field_map[:collection_id], obj_extract("contains_contained_by", "to_a", "container_id")


    # Note standard created_at, updated_at, and published are duplicated
    # in CollectionIndexer. Maybe we want to DRY it somehow.
    to_field field_map[:created_at] do |rec, acc|
      if rec.created_at
        acc << rec.created_at.utc.iso8601
      end
    end

    to_field field_map[:updated_at] do |rec, acc|
      if rec.updated_at
    ...
    to_field field_map[:published_at] do |rec, acc|
      if rec.published_at
    ...
    to_field field_map[:published_boolean], obj_extract("published?")
    ...
    to_field field_map[:oh_feature_facet] do |rec, acc|
    ...
    to_field field_map[:oh_availability_facet] do |rec, acc|
    ...
    to_field field_map[:oh_institution_facet],
    ...
    to_field field_map[:oh_birth_country_facet], obj_extract("oral_history_content", "interviewee_biographies", "to_a", "birth", "country_name")
    ...
    to_field field_map[:fulltext_en] do |rec, acc|
    ...
    to_field field_map[:fulltext_de] do |rec, acc|
    ...
    to_field field_map[:fulltext_agnostic] do |rec, acc|
    ...
    to_field field_map[:bredig_feature_facet] do |rec, acc|
