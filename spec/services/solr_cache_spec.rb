...
describe MoreLikeThisGetter,  solr: true, indexable_callbacks: true, queue_adapter: :inline do
  ...
  context "reload the core before each test" do
    # TEST B currently fails if you comment out this `before` clause,
    # which reloads the solr core (and thus clears the document cache).
    #
    # TODO figure out a smoother way to do this.
    before do
      uri = URI('http://127.0.0.1:8989/solr/admin/cores?action=RELOAD&core=scihist_digicoll_test')
      response = Net::HTTP.get(uri)
    end

    context "TEST A: limit to 2" do
      let(:limit) { 2 }
    ...
    context "TEST B: default limit, which is 5" do
      let(:limit) { 5 }
