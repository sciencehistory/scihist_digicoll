...
describe OhmsTranscriptDisplay, type: :presenter do
  ...
  it "produces good html" do
    ...
    expect(parsed.css("a.ohms-transcript-timestamp").count).to eq(ohms_xml.sync_timecodes.count)

    first_line = parsed.at_css("div.ohms-transcript-container > p.ohms-transcript-paragraph:first-child > span.ohms-transcript-line:first-child")
    expect(first_line.to_html).to eq(
      %Q{<span class="ohms-transcript-line" id="ohms_line_1"><span class="ohms-speaker">BROCK:</span> This is an oral history interview with Ron Duarte taking place on 13 June \n</span>}
    )
  end
end
